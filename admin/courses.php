...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($courseId) {
        ...
        if (!$course) {
            $errors[] = 'The selected course no longer exists.';
        }
    }
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if (!$errors) {
            ...
            if (!move_uploaded_file($_FILES['image']['tmp_name'], __DIR__ . '/../assets/images/' . $image)) {
    ...
    if (!$errors) {
        ...
        if ($courseId) {
            $db->update('courses', $data, 'id = ?', [$courseId]);
        } else {
...
<head>
    ...
    <title>Course Management | ICTECH</title>
    ...
    <aside class="admin-sidebar">
        <div class="admin-sidebar-inner">
            ...
            <div class="admin-user-box">
                <p class="name"><?php echo h(Auth::getCurrentUser()['name'] ?? 'Administrator'); ?></p>
            ...
            <div class="admin-topbar-inner">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <form method="post" action="../logout.php" class="d-inline">
                        <input type="hidden" name="csrf_token" value="<?php echo h(Auth::generateCSRFToken()); ?>">
                        <button type="submit" class="btn btn-sm btn-outline-primary"><i class="fas fa-sign-out-alt"></i> Logout</button>
                    </form>
                </div>
            </div>
        </header>
        ...
        <div class="admin-content-body">
            <main class="container-fluid px-0">
                ...
                <section class="card mb-5">
                    <div class="card-body">
                        <h2 class="h4"><?php echo $course ? 'Edit Course' : 'Add Course'; ?></h2>
                        <form method="post" enctype="multipart/form-data" class="row g-3">
                            ...
                            <div class="col-md-8">
                                ...
                                <input class="form-control" id="course-title" name="title" value="<?php echo h($course['title'] ?? ''); ?>" required>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="category_id" required>
                                    ...
                                    <?php foreach ($categories as $category): ?>
                                        <option value="<?php echo (int) $category['id']; ?>" <?php echo (($course['category_id'] ?? '') == $category['id']) ? 'selected' : ''; ?>>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" name="description" rows="3"><?php echo h($course['description'] ?? ''); ?></textarea>
                            </div>

                            <div class="col-12">
                                <label class="form-label">Full course outline</label>
                                <textarea class="form-control" name="course_outline" rows="6" placeholder="Module 1: ...&#10;Module 2: ..."><?php echo h($course['course_outline'] ?? ''); ?></textarea>
                                <small class="text-muted">Add one topic/module per line for best display on the public course page.</small>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">Learning objectives</label>
                                <textarea class="form-control" name="objectives" rows="3"><?php echo h($course['objectives'] ?? ''); ?></textarea>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">Requirements</label>
                                <textarea class="form-control" name="requirements" rows="3"><?php echo h($course['requirements'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" name="is_featured" value="1" <?php echo !empty($course['is_featured']) ? 'checked' : ''; ?>> Featured course
                                </label>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <?php if ($course): ?>
                                    <a href="courses.php" class="btn btn-link">Cancel edit</a>
                                <?php endif; ?>
                            </div>
                        </form>
                    </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h2 class="h4 mb-0">All Courses</h2>
                    <span class="text-muted"><?php echo count($courses); ?> records</span>
                </div>
                        ...
                        <?php foreach ($courses as $item): ?>
                            <tr>
                                ...
                                <td class="text-nowrap">
                                    <a href="courses.php?edit=<?php echo (int) $item['id']; ?>" class="btn btn-sm btn-outline-primary">Edit</a>
                                    <form method="post" class="d-inline">
                                        <input type="hidden" name="csrf_token" value="<?php echo h(Auth::generateCSRFToken()); ?>">
                                        <input type="hidden" name="course_id" value="<?php echo (int) $item['id']; ?>">
                                        <button name="action" value="archive" class="btn btn-sm btn-outline-secondary">Archive</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
